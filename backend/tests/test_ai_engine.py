...
def test_constraint_manager_conflict_detection(sample_constraints):
    ...
    timetable = {
        'monday': {
            ...
            2: {
                'teacher_id': 'T1',  # Teacher conflict
                'course_id': 'C2',
                'room_id': 'R2',
                'class_id': 'Class2',
                'room_type': 'lab'
...
def test_visiting_faculty_constraint():
    constraints = [{
        ...
        'max_consecutive': 2,
        'is_visiting': True,
        'availability': [1, 2]  # Only available timeslots 1 and 2
    }]
    
    cm = ConstraintManager(constraints)
    timetable = {
        'monday': {
            3: {  # Timeslot 3 is outside availability
                'teacher_id': 'VT1',
                'course_id': 'C1',
                'room_id': 'R1',
                'class_id': 'Class1',
...
def test_room_type_constraint():
    constraints = [{
        ...
        'class_id': 'Class1',
        'timeslot': 1,
        'room_type': 'lab',  # Requires lab room
        'max_hours': 20,
        'max_consecutive': 3,
        'is_visiting': False,
        'availability': []
    ...
    timetable = {
        'monday': {
            1: {
                ...
                'room_id': 'R1',
                'class_id': 'Class1',
                'room_type': 'classroom'  # Wrong room type
            }
        }
    }
